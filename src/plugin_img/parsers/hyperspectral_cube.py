...
class HyperspectralCube:
        ...
        fig.update_layout(
            ...
            updatemenus=[
                dict(
                    ...
                    direction="right",
                    buttons=[
                        dict(label="RGB", method="update",
                             args=[{"visible": [True, False, False, False]}]),
                        dict(label="R", method="update",
                             args=[{"visible": [False, True, False, False]}]),
                        dict(label="G", method="update",
    ...
    def plot_wavelength_images(
        ...
        fig = make_subplots(
            ...
            cols=cols,
            subplot_titles=[
                f"{wl:.1f} nm" for wl in wavelength_list
            ]
        )
...
if __name__ == "__main__":
    ...
    wavelengths = np.array(
        [float(w) for w in hdr["wavelength"].strip("{}").split(",")]
    )
    ...
    fig.show()
    fig = cube.plot_wavelength_images([350, 450, 550, 650, 750, 850, 950, 1050], max_cols=3)
    fig.show()
